...
``
statix : { actionName, encryptedTokenFile, disabled ? [], ignore ? [] } -> App
```
...
```
clippy : { actionName, encryptedTokenFile } -> App
```
...
```
reviewDog : { linter, errorFormat ? "%f:$l%:%c: %m", format ? null, encryptedTokenFile } -> App
```
